...
public class SearchEventFromYelp implements SearchEventFromExternalClient {
  ...
  private static final String API_KEY =
      "Mz4oevIDTzdPJTKqNBDQJyrpgvFL14ZPAk-EPxoKMC55jgurf1aOdp4mZMogJniTWUvoGn7S2eq0DCCdREKWF_gSyHbN2GpzL92k1kf1DCHo5c_GkkFy0-jLcv_xXXYx";
  ...
  public Item[] searchEvents(double lat, double lon, String category, int limit, int offset)
    ...
    String query =
        String.format(
            "sort_by=%s&categories=%s&latitude=%s&longitude=%s&limit=%s&offset=%s&radius=%s",
            SORT_BY, category, lat, lon, limit, offset, DEFAULT_RADIUS);
    ...
    try {
      ...
      itemArray =
          new ObjectMapper()
              .readValue(jsonObject.getJSONArray("businesses").toString(), Item[].class);
